...
class StartDart {
  ...
  StartDart({
    ...
    newGameData = GameData(
        numColors, populationSize, codeLength, secretCode, numGenerations, 0.1);
  }

  Future<List<List<List<int>>>> geneticAlgorithm() async {
    List<List<int>> population = List.generate(
        newGameData.populationSize,
        (_) => gameHelper.generateRandomCode(
            newGameData.numColors, newGameData.codeLength));
    List<int> temp = gameHelper.generateRandomCode(
        newGameData.numColors, newGameData.codeLength);
    List<List<List<int>>> guessList = [
      [
        gameHelper.hint(temp, newGameData.secretCode, newGameData.codeLength,
            newGameData.numColors),
        temp
      ]
    ];
    ...
    for (int generation = 0;
        generation < newGameData.numGenerations;
        generation++) {
      ...
      for (var individual in population) {
        ...
        for (var g in guessList) {
          var h = gameHelper.hint(individual, g.last, newGameData.codeLength,
              newGameData.numColors);
          var h_i = gameHelper.hint(g.last, newGameData.secretCode,
              newGameData.codeLength, newGameData.numColors);
      ...
      List<List<dynamic>> populationWithFitness = List.generate(
          population.length,
          (index) => [fitnessList[index], population[index]]);
      ...
      if (populationWithFitness.first[0] == 0) {
        ...
        var newHint = gameHelper.hint(newGuess, newGameData.secretCode,
            newGameData.codeLength, newGameData.numColors);
      ...
      if (gameHelper.hint(guessList.last.last, newGameData.secretCode,
              newGameData.codeLength, newGameData.numColors)[0] ==
          newGameData.codeLength) {
        break;
      }
      ...
      population =
          populationWithFitness.map((e) => List<int>.from(e[1])).toList();
      ...
      List<List<int>> transpositionPopulationList =
          transpositionPopulation.map((e) => gameHelper.transpose(e)).toList();
      ...
      while (i < crossoverSize) {
        ...
        var crossover = gameHelper.crossover(
            crossoverPopulation.elementAt(randomIndex1),
            crossoverPopulation.elementAt(randomIndex2),
            newGameData.codeLength);
      ...
      List<List<int>> mutatePopulationList =
          mutatePopulation.map((e) => gameHelper.circularMutate(e)).toList();
      population += [
        ...transpositionPopulationList,
        ...crossoverPopulationList,
